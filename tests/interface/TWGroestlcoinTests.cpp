...
TEST(Groestlcoin, Address) {
    ...
    auto address = TWGroestlcoinAddress();
    TWGroestlcoinAddressInitWithPublicKey(&address, publicKey, TWP2PKHPrefixGroestlcoin);
    auto addressString = WRAPS(TWGroestlcoinAddressDescription(address));
...
TEST(Groestlcoin, ExtendedKeys) {
    ...
    // .bip49
    auto mptv = WRAPS(TWHDWalletGetExtendedPrivateKey(wallet.get(), TWPurposeBIP49, TWCoinTypeGroestlcoin, TWHDVersionYPRV));
    auto mtub = WRAPS(TWHDWalletGetExtendedPublicKey(wallet.get(), TWPurposeBIP49, TWCoinTypeGroestlcoin, TWHDVersionYPUB));

    assertStringsEqual(mptv, "yprvAJkRD9AD6QrU1hvSdcJT1Cdc1DwEMsBHFt4Gqd5NsK8Vhdn3ArEHYGaJhWotcn24VWx9rC6dDutHNea9zws8owL1qWEt3pVD2GGk4DSXyvm");
    assertStringsEqual(mtub, "ypub6Xjmceh6vnQmEBzujdqTNLaLZFmimKu8d6yse1UzRefUaS7BiPYY64tnYpQQydp1gnb2cGkccBd1RtHRDtGXagqmRLxTStV88GWaeYh8ndG");
...
TEST(Groestlcoin, DeriveFromZpub) {
    ...
    auto pubKey4 = TWHDWalletGetPublicKeyFromExtended(zpub.get(), TWCurveSECP256k1groestl, TWHDVersionZPUB, TWHDVersionZPRV, 0, 4);
    auto pubKey11 = TWHDWalletGetPublicKeyFromExtended(zpub.get(), TWCurveSECP256k1groestl, TWHDVersionZPUB, TWHDVersionZPRV, 0, 11);
